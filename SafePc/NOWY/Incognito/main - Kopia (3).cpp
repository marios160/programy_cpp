...
void pobierz()
{
	...
	if (tab[3].substr(1) != czas())
	{
		loguj("Inna data, resetowanie limitu");
		tab[3] = "*" + czas();															//zapisujemy aktualna date
		aktlimit = limit*60;
	}
...
{	
	...
	for(int i=0;i<5;i++)
	{
			plik << koduj(tab[i]);										//jesli pierwsza linia to bez entera		
	}		   
    plik.close(); 
	wyslijFTP(path,computer+user); 
}
//------------------------------------------------------
/*liczenie czasu*/
...
void licz()
{
    ...
    {   																			//petla sie wykomnuje										                                                                                           
        	...
        	pobierz();				//pobieramy do tablicy dane z pliku		                                 							//odejmuje od czasu dzialania 5 sek
			...
			wyslij();								//i wysylamy aktualne dane do pliku
    }
    perror(1);
}
...
int main()
{
	...
	pathname();											//sprawdzamy nazwe uzytkownika i sciezke
	pobierzFTP(computer+user,sciezka);											//pobieramy plik z FTP
	tab = new string[5];														//tworzymy tablice																			
	pobierz();											//pobieramy dane
	
	
   	
	if (czas() == tab[3].substr(1))			//jesli data jest taka sama jak aktualna
	{										//to
	...
	{	
		...
		tab[3]="*"+czas();					//aktualna date
		wyslij();							//wysylamy to do pliku	
